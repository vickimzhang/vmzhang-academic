...
links:
  - icon_pack: fab
    ...
    url: 'https://www.notion.so/vmzhang/How-I-used-blogdown-RStudio-to-create-my-own-website-4be20bc964944aeaad4289a2df7ca21c'
...
- multiple [separate pages](https://www.vickimzhang.rbind.io/post/hugo-blogdown/#making-new-separate-widget-pages)
- a way to filter by tags (especially for research and posts)


Also, some other tips and tricks that helped me (also linked):

- Viewing [locally](https://vickimzhang.rbind.io/post/hugo-blogdown/#run-locally-to-see-how-your-website-looks9)
...
Okay. Now, about those widgets. This part stumped me for the better part of an afternoon.
...
One page down, one more to go! I basically repeated this process to create my "Outreach" page, except using other widgets that I wanted. Learning to create a separate page was definitely an uphill battle, but after figuring that out (and become familiar with `blogdown` and my repo), it was smoother sailing to create more pages. I say "smoother" because the next challenge was creating a "Posts" and a "CV" page. More on that below.

### Filtering posts by tags/categories

I was able to make my [research projects filterable by tags](https://vickimzhang.rbind.io/research/#ugprojects) and my [publications filterable by publication type](https://vickimzhang.rbind.io/research/publications) in a drop-down menu. I still cannot figure out how to achieve the same thing with posts; I'm envisioning a drop-down menu where I can select "Month/Year" or "Category" and then only posts that match the date or category will pop up. Perhaps the "Posts" page for blog posts is inherently different than "Projects", but also this makes no sense because static pages are static pages are static pages, right? Please [contact me]({{< ref "/home/contact" >}}) if you have any insights.

In any case, I linked to a page that displays all my blog posts' tags and categories by referencing to the `tags` and `categories` path. The code to reference something (e.g., blog post) within your site is `{{< ref "/post/blog-post" >}}`. The part in quotes is the permalink, or slug, to your blog post. As long as one of your blogs has that URL, then you are able to reference it anywhere in your site. You can link this by using the Markdown syntax `[linked text](link)`. So, putting it all together, I referenced blog categories and tags like this: `[category]({{< ref "/categories" >}})` and `[tags]({{< ref "/tags" >}})`. To keep track of the dates which I published posts, I assigned each month to another tag. For example, I added "2020-May" as a tag to denote posts published during the month of May in 2020.
